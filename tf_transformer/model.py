...
def sublayer_connection(inputs, sublayer, dropout=0.1):
...
class MultiHeadAttention(tf.keras.Model):
    ...
    def scaled_dot_product_attention(self, query, key, value, masked=False):
        ...
        attention_map = tf.nn.softmax(outputs) # o3 = tf.nn.softmax(o2)

        return tf.matmul(attention_map, value) # tf.matmul(o3, vs)
...
def Model(features, labels, mode, params):
    ...
    loss = tf.losses.mean_squared_error(labels=labels, predictions=decoder_outputs)
